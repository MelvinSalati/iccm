...
const StatusBadge = ({ status }: { status: Appointment['status'] }) => {
    ...
    return (
        <Badge variant="outline" className={cn('px-2 py-0.5 text-[10px] font-medium', config.color)}>
            {config.label}
        </Badge>
    );
};
...
// Main Appointment Table Component - Compact
            ...
            <div className="flex flex-col sm:flex-row gap-2 items-start sm:items-center justify-between">
                <div className="flex flex-wrap gap-1.5">
                    ...
                    <Button
                        ...
                        size="sm"
                        onClick={() => { setFilterPeriod('weekly'); setCurrentPage(1); }}
                        className="h-7 text-xs px-3"
                    >
                    ...
                    <Button
                        ...
                        size="sm"
                        onClick={() => { setFilterPeriod('monthly'); setCurrentPage(1); }}
                        className="h-7 text-xs px-3"
                    >
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="bg-slate-50">
                                <TableHead className="w-[150px] text-xs font-medium">Patient</TableHead>
                                <TableHead className="w-[130px] text-xs font-medium">Date & Time</TableHead>
                                <TableHead className="w-[110px] text-xs font-medium">Type</TableHead>
                            ...
                            {paginatedAppointments.length === 0 ? (
                                ...
                                paginatedAppointments.map((appointment) => (
                                    <TableRow key={appointment.id} className="hover:bg-slate-50">
                                        ...
                                        <TableCell>
                                            <span className="text-xs truncate block max-w-[90px]">{appointment.type || '—'}</span>
                                        </TableCell>
                                        <TableCell>
                                            <StatusBadge status={appointment.status} />
                                        </TableCell>
